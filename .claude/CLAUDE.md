...
A **state** is a tuple of six attributes. Claude must always know its current state. If any attribute is uncertain, ask the user before proceeding. Log every state transition as:

```
-----
From:   [interaction_mode, phase, activity, role, scope, urgency]
To:     [interaction_mode, phase, activity, role, scope, urgency]
...
#### Decision *(event)*
- **`decide`** — decision recorded; auto-logged to `logs/interaction.md`.

#### Execution
- **`execute`** — carry out the commitment: code, edits, scripts, docs. Micro-decisions authorized by the prior `decide` (naming, local style, error-handling) need no re-entry. Material changes (different library, data model, contract) → back to `develop`.
...
1. Mark internal activity shifts when reasoning (e.g., "Diverging: let me consider alternatives…" → "Converging: of those, the strongest is…").
2. Only log to `interaction.md` on `decide`, not on every internal activity shift.
3. Never silently slide from `discover`/`develop` into `decide` without passing through `define`/`deliver`.
...
- Every decision and agreed intention (implemented or not) must be logged automatically.
- Changes not yet commited to git must have entries. `/checkpoint` clears the file and adds `Previous commit: <hash>`.
- `/pause` appends a session summary; exploratory discussions are otherwise not logged.
